...
class PN532:
    ...
    def mifare_classic_write_block(self, block_number, data):
        """Write a block of data to a MIFARE Classic card. Block number should be the block
        to write, and data should be a byte array of length 16. Returns True if the block
        was written successfully, otherwise False.
        """
        assert data is not None and len(data) == 16, 'Data must be an array of 16 bytes!'
        # Build parameters for InDataExchange command to write MIFARE Classic block
        params = bytearray(3 + len(data))
        params[0] = 0x01  # Max card numbers
        params[1] = MIFARE_CMD_WRITE  # Write command (0xA0)
        params[2] = block_number & 0xFF
        params[3:] = data
        # Send InDataExchange request
        response = self.call_function(_COMMAND_INDATAEXCHANGE,
